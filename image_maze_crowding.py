...
# Learning curve
avg_dist = [np.mean(distances_log[i:(i + 1) * (num_populations * 2 )]) for i in range(num_generations)]
max_dist = [np.max(distances_log[i:(i + 1) * (num_populations * 2)]) for i in range(num_generations)]
plt.figure()
...
plt.savefig(f'./resources/{env_id.split(":")[1]}_crowding_heatmap.png')
